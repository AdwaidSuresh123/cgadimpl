...
namespace ag::jit
{
    ...
    struct Compiled::Impl
    {
        ...
        static const Tensor &as_ref(const Arg &a,
                                    ...
                                    Tensor &tmp)
        {
            if (std::holds_alternative<ArgInput>(a))
                return *inputs[std::get<ArgInput>(a).idx];
            if (std::holds_alternative<ArgParam>(a))
                return *params[std::get<ArgParam>(a).idx];
            if (std::holds_alternative<ArgSlot>(a))
                return slots[std::get<ArgSlot>(a).slot];
        ...
        static Tensor apply(Op op, const std::vector<const Tensor *> &a)
        {
            ...
            case Op::Transpose:
                return a[0]->transpose(-2, -1);
            case Op::Relu:
            {
                cudaStream_t stream = (cudaStream_t)ag::current_stream();
                return (*a[0] + OwnTensor::abs(*a[0], stream)) * 0.5f;
            }
            case Op::Exp:
                return OwnTensor::exp(*a[0]);
            case Op::Log:
                return OwnTensor::log(*a[0]);
            case Op::Tanh:
                return OwnTensor::tanh(*a[0]);

            case Op::MatMul:
                return OwnTensor::matmul(*a[0], *a[1]);

            // Reductions need to be updated to the new API
            case Op::Sum:
                return OwnTensor::reduce_sum(*a[0]);
            case Op::RowSum:
                return OwnTensor::reduce_sum(*a[0], {1}, true);
            case Op::RowMax:
                return OwnTensor::reduce_max(*a[0], {1}, true);
            case Op::MeanAll:
                return OwnTensor::reduce_mean(*a[0]);
        ...
        bool run(const std::vector<Tensor *> &inputs,
                 ...
                 Tensor &out) const
        {
            if (!plan.sig.matches(inputs, params))
                return false;
            ...
            for (const Step &st : plan.steps)
            {
                ...
                std::vector<Tensor> tmp_keep;
                tmp_keep.reserve(st.args.size());
    ...
    static std::string opToNovaOp(Op op)
    {
        ...
        case Op::MatMul:
            return "nova.matmul";
        case Op::Sum:
            return "nova.reduce<sum>";
        case Op::MeanAll:
            return "nova.reduce<mean>";
        default:
            return "nova.unknown_op";
        }
    }
    ...
    static std::string emitMLIR(const Plan &plan)
    {
        ...
        auto print_arg_meta = [&](const std::vector<TensorMetadata> &metas)
        {
            for (size_t i = 0; i < metas.size(); ++i)
            {
                ...
                if (i < metas.size() - 1 || !plan.sig.param_meta.empty())
                    ss << "";
            }
        };
                     ...
                     const CompileOptions &)
    {
        ...
        for (size_t i = 0; i < inputs.size(); ++i)
            in_ix[inputs[i].node.get()] = i;
        for (size_t i = 0; i < params.size(); ++i)
            par_ix[params[i].node.get()] = i;
        ...
        for (Node *n : order)
        {
            ...
            for (auto &pin : n->inputs)
            {
                ...
                {
                    if (is_in(in_ix, p))
                        st.args.push_back(ArgInput{in_ix[p]});
                    else if (is_in(par_ix, p))
                        st.args.push_back(ArgParam{par_ix[p]});
                    else
                        st.args.push_back(ArgLit{p->value});
                }
                else
                {
    ...
    extern "C" void *ABIAdapter(void **args)
    {
        ...
        for (size_t i = 0; i < num_inputs; ++i)
        {
            ...
            for (size_t j = 0; j < plan.sig.in_meta[i].shape.size(); ++j)
            {
                ...
                if (j < plan.sig.in_meta[i].shape.size() - 1)
                    std::cout << "x";
            }
            std::cout << ")\n";
        }
        ...
        for (size_t i = 0; i < num_params; ++i)
        {
            ...
            for (size_t j = 0; j < plan.sig.param_meta[i].shape.size(); ++j)
            {
                ...
                if (j < plan.sig.param_meta[i].shape.size() - 1)
                    std::cout << "x";
            }
            std::cout << ")\n";
        }
        ...
        std::cout << "[ABIAdapter] Calling compiled function with "
                  << descriptor_ptrs.size() << " arguments using libffi...\n";

        // ===================================================================
        // Use libffi for dynamic function calling (supports any arg count)
        // ===================================================================

        // Prepare libffi structures
        ffi_cif cif;
        ffi_type **arg_types = new ffi_type *[descriptor_ptrs.size()];
        void **arg_values = new void *[descriptor_ptrs.size()];

        // All arguments are pointers (void*)
        for (size_t i = 0; i < descriptor_ptrs.size(); ++i)
        {
            arg_types[i] = &ffi_type_pointer;
            arg_values[i] = &descriptor_ptrs[i];
        }

        // Prepare the call interface (returns void)
        ...
        if (status != FFI_OK)
        {
            std::cerr << "[ABIAdapter] Error: ffi_prep_cif failed with status "
                      << status << "\n";
            delete[] arg_types;
            delete[] arg_values;
            return nullptr;
        ...
        ffi_call(&cif, FFI_FN(g_aot_context.ciface_func), nullptr, arg_values);
