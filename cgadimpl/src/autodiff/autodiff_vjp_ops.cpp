...
//helper function for broadcasting issues
...
void vjp_Add(Node* n, const Tensor& gy){
    ...
    if (A->requires_grad()) A->accumulate_grad(reduce_for_broadcast(gy, A->value));
    if (B->requires_grad()) B->accumulate_grad(reduce_for_broadcast(gy, B->value));
}
...
void vjp_Mul(Node* n, const Tensor& gy){
    Node* A = n->inputs[0].get(); 
    Node* B = n->inputs[1].get();
    if (A->requires_grad()) A->accumulate_grad(reduce_for_broadcast(gy * B->value, A->value));
    if (B->requires_grad()) B->accumulate_grad(reduce_for_broadcast(gy * A->value, B->value));
}
...
void vjp_Div(Node* n, const Tensor& gy){
    ...
    if (A_node->requires_grad()) A_node->accumulate_grad(reduce_for_broadcast(gy / B_node->value, A_node->value));
    if (B_node->requires_grad()) B_node->accumulate_grad(reduce_for_broadcast(gy * A / (B * B), B));
}
...
void vjp_GELU(Node* n, const Tensor& gy){
    ...
    if (X->requires_grad()) {
        ...
        Tensor x3 = x * x * x;
        Tensor u = c1 * (x + c2 * x3);
        Tensor tu = OwnTensor::trig::tanh(u);
        Tensor d_tu = 1.0f - tu * tu;
        Tensor du_dx = c1 * (1.0f + 3.0f * c2 * x * x);
        X->accumulate_grad(gy * (0.5f * (1.0f + tu) + 0.5f * x * d_tu * du_dx));
    }
}
...
void vjp_SiLU(Node* n, const Tensor& gy){
    ...
    if (X->requires_grad()) {
        ...
        X->accumulate_grad( gy * (sig + x * sig * (1.0f - sig)));
    }
}
...
void vjp_Mish(Node* n, const Tensor& gy){
    ...
    if (X->requires_grad()) {
        ...
        Tensor tsp = OwnTensor::trig::tanh(sp);
        Tensor sig = 1.0f / (1.0f + OwnTensor::exp(x * -1.0f, ag::current_stream()));
        X->accumulate_grad( gy * (tsp + x * (1.0f - tsp * tsp) * sig));
    }
}
...
void vjp_Gaus(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad( gy * n->value * (X->value * -2.0f));
}

void vjp_Parcon(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) {
        X->accumulate_grad(gy * (2.0f - X->value * 2.0f)) ;
    }
}
...
void vjp_LiSHT(Node* n, const Tensor& gy){
    ...
    if (X->requires_grad()) {
        Tensor x = X->value;
        Tensor tx = OwnTensor::trig::tanh(x);
        X->accumulate_grad(gy * (tx + x * (1.0f - tx * tx)));
    }
}
// Standard Attention ---------
...
void vjp_Exp(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad(gy * n->value) ;
}

void vjp_Log(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad( gy / X->value);
}


void vjp_Sqrt(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad(gy * 0.5f / n->value);
}

void vjp_Reciprocal(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad(gy / (X->value * X->value));
}

void vjp_Sign(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad(gy * 0.0f);
}
void vjp_Abs(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) {
        const float epsilon = 1e-9f;
        X->accumulate_grad(gy * (X->value / (n->value + epsilon)));
    }
}
void vjp_Pow(Node* n, const Tensor& gy){
    ...
    if (A_node->requires_grad()) {
        A_node->accumulate_grad(reduce_for_broadcast(gy * B * OwnTensor::exp((B - 1.0f) * OwnTensor::log(A, ag::current_stream()), ag::current_stream()), A));
    }
    if (B_node->requires_grad()) {
        B_node->accumulate_grad(reduce_for_broadcast(gy * n->value * OwnTensor::log(A, ag::current_stream()), B));
    }
}
...
void vjp_MatMul(Node* n, const Tensor& gy){
    Node* A = n->inputs[0].get();
    Node* B = n->inputs[1].get();
    if (A->requires_grad()) A->accumulate_grad(OwnTensor::matmul(gy, B->value.t()));
    if (B->requires_grad()) B->accumulate_grad(OwnTensor::matmul(A->value.t(), gy));
}

void vjp_Transpose(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad(gy.t());
}
...
void vjp_FMA(Node* n, const Tensor& gy){
    ...
    if (A->requires_grad()) A->accumulate_grad(OwnTensor::matmul(gy, B->value.t()));
    if (B->requires_grad()) B->accumulate_grad(OwnTensor::matmul(A->value.t(), gy));
    if (C->requires_grad()) C->accumulate_grad(gy);
}

void vjp_Flatten(Node* n, const Tensor& gy) {
    Node* X_node = n->inputs[0].get();
    if (!X_node->requires_grad()) return;
    const Shape& original_shape = X_node->value.shape();

    Tensor reshaped_grad = gy.reshape(original_shape);

    if (X_node->requires_grad()) {
        std::lock_guard<std::mutex> lock(X_node->grad_mutex);
        X_node->grad += reshaped_grad;
    }
}

void vjp_Dropout(Node* n, const Tensor& gy) {
    Node* X_node = n->inputs[0].get();
    if (!X_node->requires_grad()) return;
...
void vjp_KLDivergence(Node* n, const Tensor& gy){
    ...
    if (Y_node->requires_grad()) {
        ...
        Y_node->accumulate_grad(gy * (log_Y + 1.0f - log_softmax_z) * inv_batch_size);
    }
}
...
void vjp_MSELoss(Node* n, const Tensor& gy){
    ...
    if (Z_node->requires_grad()) Z_node->accumulate_grad(diff * (gy_scalar * scale));
    if (Y_node->requires_grad()) Y_node->accumulate_grad(diff * (gy_scalar * scale));
}
...
void vjp_MAELoss(Node* n, const Tensor& gy){
    ...
    if (Z_node->requires_grad()) Z_node->accumulate_grad(gy * sign_diff * inv_N);
    if (Y_node->requires_grad()) Y_node->accumulate_grad(gy * sign_diff * inv_N);
}
...
void vjp_BinaryCrossEntropy(Node* n, const Tensor& gy) {
    ...
    // Gradient formula: gy * (y_pred - y_true) / (y_pred * (1 - y_pred))
    float eps = 1e-7f;
    Tensor grad_input = gy * (y_pred - y_true) / (y_pred * (1.0f - y_pred) + eps);

    if (Y_pred_node->requires_grad()) {
        std::lock_guard<std::mutex> lock(Y_pred_node->grad_mutex);
        Y_pred_node->grad += grad_input;
    }
}
...
void vjp_CategoricalCrossEntropy(Node* n, const Tensor& gy) {
    ...
    if (!Y_pred_node->requires_grad()) return;
...
void vjp_LayerNorm(Node* n, const Tensor& gy){
    ...
    if (X->requires_grad()) {
        ...
        X->accumulate_grad(d_xhat + d_var * (x - mean) * (2.0f / static_cast<float>(D)) + d_mean / static_cast<float>(D));
    }
}
...
void vjp_RMSNorm(Node* n, const Tensor& gy){
    ...
    if (X->requires_grad()) {
        ...
        X->accumulate_grad(rsqrt_var * (gy - x * (rsqrt_var * rsqrt_var) * dot / static_cast<float>(D)));
    }
}
...
void vjp_Dyntanh(Node* n, const Tensor& gy){
    ...
    Tensor th = OwnTensor::trig::tanh(h);
    Tensor d_th = 1.0f - th * th;
    if (X->requires_grad()) X->accumulate_grad(gy * G->value * d_th * A->value);
    if (A->requires_grad()) A->accumulate_grad(OwnTensor::reduce_sum(gy * G->value * d_th * X->value, {}, false));
    if (B->requires_grad()) B->accumulate_grad(OwnTensor::reduce_sum(gy, {}, false));
    if (G->requires_grad()) G->accumulate_grad(OwnTensor::reduce_sum(gy * th, {}, false));
}

//Global Reductions -------------------
void vjp_Sum(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad(gy);
}
void vjp_MeanAll(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) {
        float scale = 1.0f / static_cast<float>(X->value.numel());
        X->accumulate_grad(gy * scale);
    }
}
//Row-wise Reductions ------------------------
...
void vjp_LogSumExpRow(Node* n, const Tensor& gy){
    ...
    if (Z->requires_grad()) {
        ...
        Tensor sum_exp_z = OwnTensor::reduce_sum(exp_z, {-1}, true);
        Tensor softmax_z = exp_z / sum_exp_z;
        Z->accumulate_grad(gy * softmax_z);
    }
}
...
void vjp_Sin(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad(gy * OwnTensor::trig::cos(X->value));
}

void vjp_Cos(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad(gy * OwnTensor::trig::sin(X->value));
}

void vjp_Tan(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) {
        Tensor c = OwnTensor::trig::cos(X->value);
        X->accumulate_grad(gy / (c * c));
    }
}
...
void vjp_Cosh(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad(gy * OwnTensor::trig::sinh(X->value));
}
void vjp_Sinh(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad(gy * OwnTensor::trig::cosh(X->value));
}

//Inverse Trigonometric Functions --------------
void vjp_Asin(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad(gy / OwnTensor::sqrt(1.0f - X->value * X->value, ag::current_stream()));
}
...
void vjp_ASinh(Node* n, const Tensor& gy){
    Node* X = n->inputs[0].get();
    if (X->requires_grad()) X->accumulate_grad(gy / OwnTensor::sqrt(X->value * X->value + 1.0f, ag::current_stream()));
}
